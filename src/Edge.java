public class Edge {
	...
	public String getString(String original) throws Exception {
		return original.substring(getstartIndex(), getEdgelength()
				+ getstartIndex());
	}
	...
	public Edge partitionEdge(int index, Node root, char c_new_edge)
			throws Exception {
		Node router = new Node(root);
		Node endnode_old = this.endNode;
		Edge routerToEndNode = new Edge(router, this.endNode, index
		...
		router.setParentEdge(this);
		// this has to be done after modifying edge, otherwise depth will be set
		// wrong.

		endnode_old.setParentEdge(routerToEndNode);
		// for setting depth correctly
